...
import { format, getDaysInMonth, parseISO, getWeekOfMonth, endOfMonth } from "date-fns";
...
const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: { value: number; payload: { fullDate?: string; fullSubject?: string; completed?: number } }[]; label?: string; }) => {
    if (active && payload && payload.length) {
        return (
            <div className="bg-zinc-950 border border-zinc-800 p-3 rounded-lg shadow-xl outline-none">
                <p className="text-zinc-300 font-medium mb-1">
                    {payload[0].payload.fullDate || payload[0].payload.fullSubject || label}
                </p>
                <p className="text-emerald-400 font-semibold text-lg">
                    {payload[0].value}% <span className="text-xs text-zinc-500 font-normal">completed</span>
                </p>
    ...
    const { dailyData, weeklyData, monthlyData } = useMemo(() => {
        ...
        try {
            ...
            const dData = Array.from({ length: daysInMonth }, (_, i) => {
                ...
                const logsForDay = logs.filter((l: IDailyLog) => l.date === dateStr && l.done);
            ...
            Array.from({ length: daysInMonth }).forEach((_, i) => {
                ...
                if (weekIndex < wData.length) {
                    ...
                    const logsForDay = logs.filter((l: IDailyLog) => l.date === dateStr && l.done).length;
            ...
            const mData = activities.map(act => {
                ...
                const completedDays = logs.filter((l: IDailyLog) => l.activityId.toString() === actId && l.done).length;
                const percent = Math.round((completedDays / daysInMonth) * 100);

                return {
                    subject: act.name.length > 15 ? act.name.substring(0, 15) + '...' : act.name,
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between mb-8 gap-4">
                ...
                <div className="flex bg-zinc-950/50 p-1 rounded-lg border border-zinc-800/50">
                    ...
                    <button
                        ...
                        className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${activeTab === "weekly"
                            ? "bg-zinc-800 text-zinc-100 shadow-sm"
                            : "text-zinc-400 hover:text-zinc-300"
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${activeTab === "monthly"
                            ? "bg-zinc-800 text-zinc-100 shadow-sm"
                            : "text-zinc-400 hover:text-zinc-300"
                            }`}
                    >
                ...
                {activeTab === "daily" && (
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={dailyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <YAxis stroke="#71717a" fontSize={12} tickLine={false} axisLine={false} domain={[0, 100]} ticks={[0, 25, 50, 75, 100]} tickFormatter={(val) => `${val}%`} />
                            <Tooltip content={<CustomTooltip />} cursor={{ fill: '#27272a', opacity: 0.4 }} />
                            <Bar dataKey="percent" fill="#10b981" radius={[4, 4, 0, 0]} maxBarSize={40} />
                        </BarChart>
                    </ResponsiveContainer>
                )}
                ...
                {activeTab === "weekly" && (
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={weeklyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <YAxis stroke="#71717a" fontSize={12} tickLine={false} axisLine={false} domain={[0, 100]} ticks={[0, 25, 50, 75, 100]} tickFormatter={(val) => `${val}%`} />
                            <Tooltip content={<CustomTooltip />} cursor={{ fill: '#27272a', opacity: 0.4 }} />
                            <Bar dataKey="percent" fill="#3b82f6" radius={[4, 4, 0, 0]} maxBarSize={60} />
                        </BarChart>
                    </ResponsiveContainer>
                )}
                ...
                {activeTab === "monthly" && (
                    <ResponsiveContainer width="100%" height="100%">
                        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={monthlyData}>
                            ...
                            <Radar name="Completion Rate" dataKey="percent" stroke="#8b5cf6" fill="#8b5cf6" fillOpacity={0.3} />
