...
interface DailyChecklistProps {
    ...
    onToggleTick: (activityId: string, date: string, done: boolean) => Promise<void>;
    isLoading?: boolean;
}
...
export default function DailyChecklist({
    ...
    const handleToggle = (activityId: string, day: number, currentDone: boolean) => {
        if (isLoading) return;
        const dayStr = day.toString().padStart(2, "0");
        const dateStr = `${currentMonth}-${dayStr}`;
            ...
            <div className="overflow-x-auto custom-scrollbar flex-1 relative">
                <table className="w-full text-sm text-left border-collapse">
                    <thead className="text-xs uppercase bg-zinc-950/80 text-zinc-400 sticky top-0 z-10 backdrop-blur-sm">
                        <tr>
                            <th scope="col" className="px-6 py-4 font-semibold w-48 sticky left-0 z-20 bg-zinc-950/95 backdrop-blur-md">
                                Activity
                            </th>
                            {daysArray.map((day) => (
                                <th key={day} scope="col" className="px-2 py-4 text-center min-w-[40px] font-medium border-l border-zinc-900">
                                    {day}
                                </th>
                            ))}
                        </tr>
                    ...
                    <tbody className="divide-y divide-zinc-800">
                        {activities.map((activity) => (
                            <tr key={activity._id as unknown as string} className="hover:bg-zinc-800/20 transition-colors">
                                <th
                                    scope="row"
                                ...
                                {daysArray.map((day) => {
                                    const done = isLogDone(activity._id as unknown as string, day);
                                    return (
                                        <td key={day} className="px-2 py-3 text-center border-l border-zinc-800/50">
                                            <button
                                                onClick={() => handleToggle(activity._id as unknown as string, day, done)}
                                                disabled={isLoading}
                                                className={`w-7 h-7 mx-auto rounded flex items-center justify-center transition-all ${done
                                            ...
                                            >
                                                <Check className={`w-4 h-4 ${done ? 'opacity-100' : 'opacity-0'} transition-opacity`} />
                                            </button>
                                        </td>
                                    );
                                })}
